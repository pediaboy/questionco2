    ...
    <div>
      <div className="mb-6">
        ...
        <h2 className="text-xl font-bold font-display text-white mt-1">
          Notifikasi <span className="text-cyan-300 text-glow-cyan">Sistem</span>
        </h2>
        <p className="text-xs text-[#94A3B8] mt-1">Log pemberitahuan resmi dari server LASTQUESTION.CO.</p>
      </div>

      {loading ? (
        <div className="space-y-4">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="h-32 bg-black/40 border border-white/5 chamfer-sm animate-pulse" />
          ))}
        </div>
      ) : error ? (
        <div className="text-center py-8 bg-[#0f172a]/50 border border-dashed border-red-500/20 chamfer-sm">
          <span className="text-xs text-red-400 font-mono">[ ERROR LOADING NOTIFICATIONS ]</span>
        </div>
      ) : items.length === 0 ? (
        <div className="text-center py-12 bg-[#0f172a]/30 border border-dashed border-white/10 chamfer-sm">
          <span className="text-xs text-slate-500 font-mono">[ TIDAK ADA NOTIFIKASI ]</span>
        </div>
      ) : (
        <div className="space-y-4">
          {items.map((item) => (
            ...
            >
              ...
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] text-slate-500 font-mono">{formatIndonesianDate(item.created_at)}</span>
                {item.pinned && (
              ...
              <p className="text-white/70 text-xs font-sans leading-relaxed whitespace-pre-wrap">{item.body}</p>
            </div>
          ))}
        </div>
      )}
