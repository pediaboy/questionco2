...
// Route group layout for top-level member-account pages (/profile, /watchlist,
// /ledger, /security, /settings, /affiliate, /support-tickets) that need the
// same auth gate + header/nav chrome as /dashboard/*, but live at a root path
...
function MemberLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen max-w-md mx-auto relative">
      <SyncIndicator />
      <MemberHeader />
      <main className="pt-20 px-4 pb-28 max-w-md mx-auto">{children}</main>
      <MemberBottomNav />
    </div>
  );
}
